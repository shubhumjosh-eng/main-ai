...
export async function unlockStorage(password: string): Promise<boolean> {
  try {
    const stored = localStorage.getItem('gv-vault')
    if (!stored) {
      const salt = crypto.getRandomValues(new Uint8Array(SALT_LENGTH))
      cachedKey = await deriveKey(password, salt)
      localStorage.setItem('gv-vault-salt', btoa(String.fromCharCode(...salt)))
      localStorage.setItem('gv-vault', await encrypt('valid'))
      return true
    }
    const salt = Uint8Array.from(atob(localStorage.getItem('gv-vault-salt') || ''), c => c.charCodeAt(0))
    cachedKey = await deriveKey(password, salt)
    const testData = localStorage.getItem('gv-vault')
    if (testData) {
...
export function isVaultCreated(): boolean {
  return !!localStorage.getItem('gv-vault-salt')
}
